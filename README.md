# Teamarr + Jellyfin Automatic Sports DVR

## Schedules teams, leagues, or events from your Teamarr EPG data

TLDR:  Use teamarr to create channels, this plugin to record them.
...
This project was (mostly) vibe coded, but many hours spent on making it work well.  It is confirmed working for me and properly schedules games respecting priority, overlap, and how many connections your provider tuner provides.  You want to create channels using the "templates" in teamarr avoiding pre and post game filler, syncing that with dispatcharr (along with a script to auto-match EPG daily in dispatcharr).  You then set up scan times for each of the three components creating a daily schedule for recording games for watching later or archiving.  This is a great option when you do not have access to games such as European football or your native countries sports.  

This plugin was made to compliment teamarr/dispatcharr and fit in well aesthetically with your Jellyfin server.  
